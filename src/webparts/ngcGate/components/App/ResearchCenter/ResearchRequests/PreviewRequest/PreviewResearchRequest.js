...
function PreviewResearchRequest() {
    ...
    const ReplyUploader = (
        ...
        >
            <Button size='middle' icon={<UploadOutlined />}>Attach Files</Button>
        </Upload>
    )
    ...
    let PendingAssignee = assigneeHistoryData?.filter(a => a.Status === "Pending" && a.ToUser?.EMail?.toLowerCase() === user_data.Data?.Mail?.toLowerCase())[0];
    // check if current user is requester or not
    let IfRequester = requestData?.Author?.EMail?.toLowerCase() === user_data?.Data?.Mail?.toLowerCase();
    ...
    <>
        ...
        <div className='standard-page preview-request-container'>
            <div className="header">
                <h1>Research Request: [#{requestData?.Id || '###'}]</h1>
                <div>
                    {
                        !['Rejected', 'Acknowledge'].includes(requestData.Status) && PendingAssignee?.Status === "Pending"
                        ?   (
                                ...
                                <Timeline>
                                    ...
                                    </div>
                                    {
                                        FilteredReplys?.map((reply, i) => {
                                            return (
                                                <div className={(reply.Author?.EMail?.toLowerCase() === user_data.Data?.Mail?.toLowerCase()) ? "my-reply" : ""}>
                                                    <Timeline.Item 
                                                        key={i}
