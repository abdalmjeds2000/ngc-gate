...
const NewsForm = ({ openModal, isEditMode, item, setOpenModal }) => {
  const { user_data, setNewsList, sp_site } = useContext(AppCtx);
      ...
      <Form form={form} layout='vertical' onFinish={SubmitForm} onFinishFailed={() => message.error("Please, fill out the form correctly")}>
        <Row gutter={[20, 20]}>
          ...
          <Col sm={24} md={24} lg={12}>
            ...
            <Row gutter={10}>
              <Col xs={24} md={12}>
                <Form.Item name="IsPerson" label="Is this Person Photo ?" initialValue={isEditMode ? item.IsPerson : ''} rules={[{required: true, message: false}]} style={{marginBottom:0}}>
                  <Radio.Group options={[{value: true, label: 'Yes'}, {value: false, label: 'No'}]} />
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item name="IsDraft" label="Is Draft ?" initialValue={isEditMode ? item.IsDraft : ''} rules={[{required: true, message: false}]} style={{marginBottom:0}}>
                  <Radio.Group options={[{value: true, label: 'Yes'}, {value: false, label: 'No'}]} />
                </Form.Item>
              </Col>
            </Row>
            ...
            <img
              src={
                ...
                : filesList[0]?.response?.uploadedFiles[0]?.Path || `${sp_site}/SiteAssets/images/no_preview.png`}
              alt=''
              style={{width: '100%'}}
            />
          </Col>
